...
`copper-parse-sql-flow` is centered on parsers work in Ruby. The technical target is: Implement a Ruby parsers project for sql format conversion, using round-trip fixtures and lossless normalization checks.
...
The score is deterministic. The current threshold is 152. Risk has a penalty of 6, latency has a penalty of 3, and weight contributes a bonus of 4.
